...
def generate_secure_bytes(length, security_level='high'):
    seed_values = []
...
def generate_secure_string(length, charset='hex', security_level='high'):
    if charset == 'hex':
        return generate_secure_bytes((length + 1) // 2, security_level).hex()[:length]
    elif charset == 'base64':
        b64 = base64.b64encode(generate_secure_bytes((length * 3 + 3) // 4, security_level)).decode('ascii')
        return b64[:length]
        ...
        return ''.join(str(b % 10) for b in generate_secure_bytes(length, security_level))
    else:
        raise ValueError(f"Unknown charset: {charset}")


def generate_secure_number(bits, security_level='high'):
    bytes_needed = (bits + 7) // 8
    value = int.from_bytes(generate_secure_bytes(bytes_needed, security_level), byteorder='big')
    max_value = (1 << bits) - 1
